...
**ローカルLLM**: Ollama をホストにネイティブインストールして使用（フェーズ2）
...
│   │       │   └── llm_tagger.py   # Ollama API (フェーズ2: mode=llm)
│   │       └── search/
│   │           ├── __init__.py
│   │           ├── base.py         # SimilaritySearchStrategy (ABC)
│   │           ├── tfidf_search.py # TF-IDF（フェーズ2: mode=rule_based/ml）
│   │           └── embedding_search.py # sentence-transformers（フェーズ2: mode=llm）
│   │
...
tagger:
  ...
  llm_endpoint: http://localhost:11434  # Ollama（フェーズ2）
  llm_model: gemma3              # Ollamaモデル名（フェーズ2）
...
| パッケージ | フェーズ1 dependencies | フェーズ2追加 |
|---|---|---|
| `collector` | `httpx>=0.27` | — |
| `processor` | （なし） | `scikit-learn`, `sentence-transformers` |
...
-- タグ編集履歴（フェーズ2: 学習機能・Few-Shotプロンプト生成に使用）
...
#### tagging/llm_tagger.py（フェーズ2: `mode=llm`）
...
#### search/embedding_search.py（フェーズ2）
